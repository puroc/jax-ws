...
public class UserServiceAgentService extends Service {
    ...
    static {
        URL url = UserServiceAgentService.class.getResource("UserService.xml");
        if (url == null) {
            url = UserServiceAgentService.class.getClassLoader().getResource("UserService.xml");
        } 
        if (url == null) {
            java.util.logging.Logger.getLogger(UserServiceAgentService.class.getName())
                .log(java.util.logging.Level.INFO, 
                     "Can not initialize the default wsdl from {0}", "UserService.xml");
        }       
        WSDL_LOCATION = url;
    }
